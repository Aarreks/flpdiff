# flpdiff — development reference

Internal notes tracking how `flpdiff` was built, which opcodes were
discovered when, the parity-harness results against the reference
Python implementation, and the clean-room discipline guiding each
parser decision. The public-facing README (`../README.md`) is the
product-user entry point; this document is for people hacking on
the parser itself.

Originally grown as the project's progress tracker; preserved here
post-launch as a reference.
...
The TS implementation exists to deliver two wins:

1. **Clean-room FLP parser** — no GPL dependency, MIT-licensed, no
   Python install.
...
| #  | Bug                                                                                             | Impact                                        |
|----|-------------------------------------------------------------------------------------------------|-----------------------------------------------|
| 1  | Playlist opcode `0xD9 → 0xE9` (canonical `DATA+25`)                                              | `clips_total=0` on every FL 21+ file         |
| 2  | Orphan-clip filter (`track_rvidx > 499` or missing channel/pattern ref)                          | Clip over-count by ~3× on real files         |
| 3  | Sampler reclassification — `type=Instrument + SamplePath + empty 0xC9 → Sampler`                 | Channel kinds off by 3–5× on real files      |
...
| 11 | First-separator swallow in slot divide (push on 2nd+ `0x62`, flush at `0x93`)                   | `filled_slots` phantom-slot over-count on FL 20-25 |
...
3. **Per-insert routing projection** — the project-level
   ...
   `RouteVolStart` record to produce a `routes_to: number[]` per
   insert is deferred. Python's `flp-info` emits `routes_to: []`
   everywhere (a known limitation of the reference adapter), so
   parity doesn't force this one.
...
- MixerParams attribution: `0xE1` records unpack via
  ...
  `MixerSlot.{enabled,mix}`. Presentation layer mirrors Python's
  `slot.enabled: true` hardcoded output; the real bit is preserved
...
- Muted channel state: investigated — there is no channel-muted
  opcode; Python's `Channel.muted` defaults to False; TS
  presentation layer matches by construction. `enabled`
  (is-enabled `0x00`) is the sole audibility signal.
- Note flags bitmask: `Note.slide` now derived from `flags & 0x08`,
  the sole catalogued flag bit. Raw `flags` kept alongside for
  future bit additions and round-trip fidelity.
- Channel zipped: `Channel.zipped` decoded from `0x0F`. Absence ≡
  false; FL only emits the opcode for collapsed channels.
...
The diff engine is complete. Realistic next paths:

1. **Browser viewer** — bundle the parser + diff engine as a static
   web page. Two file-drop zones + a "Diff" button + rendered
   summary pane. Target <2 MB gzipped. Deployment via GitHub
   Pages / Cloudflare Pages.
2. **Format-spec deepening** — remaining items in the "Known open
   format work" list above: per-insert routing projection,
   clip-bearing fixtures, zipped-channel fixture. All cosmetic on
   top of the already-solid parity numbers.
3. **Plugin-state registry** — wire up a TS plugin-state decoder
   registry, then route opaque-blob diffs through it so
